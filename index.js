...
const vcard = 'BEGIN:VCARD\n' // Tarjeta de contacto
            ...
            + 'FN:mauri\n' // Nombre
            + 'ORG:mauri;\n' // Propietario
            + 'TEL;type=CELL;type=VOICE;waid=994407747291:+994 40 774 72 91\n' // ID de WhatsApp + número de teléfono
            + 'END:VCARD'
/******END OF VCARD INPUT******/
	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `eu @${num.split('@')[0]} para usar el bot registrate con el comando ${prefix}daftar y tu nombre\n\npara ver los demas comandos usa ${prefix}help`
                          client.sendMessage(mdata.id, teks, MessageType.text, { contextInfo: {"mentionedJid": [num]}})
			} else if (anu.action == 'remove') {
				num = anu.participants[0]
				teks = `dios no lo soporto a @${num.split('@')[0]}`
				client.sendMessage(mdata.id, teks, MessageType.text, {contextInfo: {"mentionedJid": [num]}})
			}
		} catch (e) {
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				levelnol: '*Nivel* 0 ',
				error: {
					stick: 'intenta de vuelta, no se pudo',
					Iv: 'no existe ese link gordo de mierda'
				},
				only: {
					group: 'este comando es solo para grupos, sos pelotudo eh',
					ownerG: 'solo el admin puede usar este comando',
					ownerB: 'este comando solo lo usa mauri',
					admin: 'este comando solo puede ser usado por los admins',
					Badmin: 'este comando solo se puede usar si el bot es admin',
                                        daftarB: `no estas registrado en la base de datos \n\ncomando : ${prefix}daftar nombre`,
				}
			}
               ...
               break*/
					...
					if (mentioned.length > 1) {
						...
						for (let _ of mentioned) {
							teks += `sacandole el admin\n`
							teks += `@_.split('@')[0]`
						}
						mentions(teks, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					} else {
						mentions(`te quedaste sin admin por boludo@${mentioned[0].split('@')[0]}\n*${groupMetadata.subject}*_`, mentioned, true)
						client.groupDemoteAdmin(from, mentioned)
					}
					break
                 case 'promote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`toma admin @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupMakeAdmin(from, mentioned)
					}
					break
				  case 'wa.me':
  ...
  client.updatePresence(from, Presence.composing) 
	...
	case 'hidetag':
              ...
              await client.sendMessage(from, options, text)
               break
                                case 'ytmp3':
					if (args.length < 1) return reply('y la url? mogolico')
					if(!isUrl(args[0]) && !args[0].includes('youtu')) return reply(mess.error.Iv)
					anu = await fetchJson(`https://api.zeks.xyz/api/ytmp3?apikey=${args[0]}&apikey=${ZeksApi}`, {method: 'get'})
					if (anu.error) return reply(anu.error)
					teks = `❏ *Título* : ${anu.title}\n❏ *Tamaño del archivo* : ${anu.result.size}\n\n ahi te envio el audio`
				...
				case 'ytmp4':
				   ...
				   if (args.length < 1) return client.sendMessage(from, 'PONE EL CODIGO DE IDIOMA GORDO DE MIERDA', text, {quoted: mek})
                                   if (!isUser) return reply(mess.only.daftarB)
				...
				case 'adminlist':
					client.updatePresence(from, Presence.composing) 
                                        if (!isUser) return reply(mess.only.daftarB)
					if (!isGroup) return reply(mess.only.group)
					teks = ` lista de los mas homosexuales de ${groupMetadata.subject}\nTotal : ${groupAdmins.length}\n\n`
					no = 0
			...
			case 'tagall':
				...
				client.updatePresence(from, Presence.composing) 
					...
					client.sendMessage(from, '╔══✪〘 Mencionar A Todos 〙✪══\n╠➥'+teks+'╚═〘 mauribot 〙', text, {detectLinks: false, quoted: mek})
					break
                        case 'tagall4':
				client.updatePresence(from, Presence.composing) 
					if (!isGroup) return reply(mess.only.group)
				...
				case 'bc':
					...
					client.groupLeave (from) 
                     ...
                     setTimeout( () => {
					client.updatePresence(from, Presence.composing) 
					client.sendMessage(from, 'chau', text) // ur cods
					}, 0)
                     break
       /*case 'ownergrup':
				  case 'ownergroup':
          ...
          contextInfo: { mentionedJid: [from] }
           ...
           client.sendMessage(from, options, text, { quoted: mek } )
				break*/
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'dios amigo no lo soporto mas a \n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`dios no te soporto @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					client.sendMessage(mentioned, 'andate hijo de re mil puta', text)
					}
					break
				case 'exe':
	              client.updatePresence(from, Presence.composing) 
	              if (!isOwner) return reply(mess.only.ownerB)
	               ...
	               exec(cmd, (err, stdout) => {
		           if(err) return client.sendMessage(from, "chau", text, { quoted: mek })
		           if (stdout) {
			...
			case 'closegc':
				...
				case 'stikergif':
			        if ((isMedia && !mek.message.videoMessage || isQuotedImage) && args.length == 0) {
						...
						await ffmpeg(`./${media}`)
							...
							.on('error', function (err) {
								...
								reply(`no se pudo convertir ${tipe} a sticker`)
							})
							.on('end', function () {
								console.log('Finish')
				    ...
				    client.updatePresence(from, Presence.composing)
                                    if (!isUser) return reply(mess.only.daftarB)
					if (!isQuotedSticker) return reply('solo se puede con stickers pelotudo')
					reply(mess.wait)
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('no pude, intenta de vuelta')
						buffer = fs.readFileSync(ran)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'no'})
						fs.unlinkSync(ran)
					})
					break
                	case 'tomp3':
                	client.updatePresence(from, Presence.composing) 
                        if (!isUser) return reply(mess.only.daftarB)
					if (!isQuotedVideo) return reply('solo se puede con videos gordo boludo')
					reply(mess.wait)
					encmedia = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					media = await client.downloadAndSaveMediaMessage(encmedia)
					ran = getRandom('.mp4')
	        ...
	        if (!isUser) return reply(mess.only.daftarB)
               ...
               if (anu.error) return reply(anu.error)
                ...
                break
                                case 'daftar':
					client.updatePresence(from, Presence.composing)
					if (isUser) return reply('ya te registraste boludo')
					...
					break
                                case 'welcome':
					if (args.length < 1) return reply('para activar esta funcion pone welcome 1')
					if (Number(args[0]) === 1) {
						if (isWelkom) return reply('activada')
						welkom.push(from)
					...
					break
                               case 'nsfwneko':
				    try{
						if (!isNsfw) return reply('el nsfw no esta activado')
                                                if (!isUser) return reply(mess.only.daftarB)
            ...
            break
				case 'fitnah':
				if (args.length < 1) return reply(`uso :\n${prefix}fitnah [@tag|texto q va a decir la persona tageada|lo q responde el bot]\n\nejemplo : \n${prefix}fitnah @ikod|q rico|dios deja de mandar porno furry`)
				var gh = body.slice(7)
				mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
            ...
            case 'leveling':
                if (!isGroup) return reply(mess.only.group)
                if (!isGroupAdmins) return reply(mess.only.admin)
                if (args.length < 1) return reply('con 1 se activa')
                if (args[0] === '1') {
                    ...
                    reply(`con 1 se activa y con 0 se desactiva \n ejemplo: ${prefix}leveling 1`)
                }
            break
                                /*case 'nsfwtrap':
                                        try{
				...
				case 'nsfwass':
					...
					case 'nsfwarmpits':
				        ...
				        uptime = process.uptime()
                                        client.sendMessage(from, `Velocidad:${latensi.toFixed(4)}`, text, { quoted: mek})
                                        break
                                case 'ttp':
